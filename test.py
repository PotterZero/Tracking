...
tracked_face = None  # Biến để lưu đối tượng khuôn mặt được "ghim"
...
while True:
    ...
    # Nếu chưa có khuôn mặt nào được ghim và có ít nhất một khuôn mặt được phát hiện
    if tracked_face is None and bboxs:
        # Lưu thông tin khuôn mặt đầu tiên được phát hiện
        tracked_face = bboxs[0]

    # Nếu đã ghim một khuôn mặt, chỉ theo dõi khuôn mặt đó
    if tracked_face:
        fx, fy = tracked_face["center"][0], tracked_face["center"][1]
        pos = [fx, fy]

        # Vẽ vòng tròn và các đường thẳng chỉ vị trí của khuôn mặt được ghim
        ...
        # Nếu không có khuôn mặt nào được ghim, hiển thị "NO TARGET"
